...
async def async_main_long(isbns:List[str]=ISBNS):
  for index, isbn in enumerate(isbns):
    await asyncio.get_event_loop().run_in_executor(None, print_book_info, index, isbn)
...
async def async_main_multi_line_loop(isbns:List[str]=ISBNS):
  for index in range(0, len(isbns), 2):
    ...
    if index + 1 < len(isbns):
      await loop.run_in_executor(None, print_book_info, index + 1, isbns[index+1])
...
def main():
  ...
  timings = [
    time_sync(f) if ftype=='sync' else time_async(f)
    for f, ftype in funcs_to_time
  ]
  print("\n\n\n")
